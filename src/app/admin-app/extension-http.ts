...
@Injectable()
export class ExtensionHttp extends Http { 
    status = {
   ...
   request(url: string | Request, options ? : RequestOptionsArgs): Observable < Response > {
     typeof url=='string'? (url=DI_CONFIG.baseApiUrl+url):(url.url=DI_CONFIG.baseApiUrl+url.url);
  ...
  handleError(status) {
    if (status<200 || status>=300) {
      this._pubsub.errorToast.next(status+' - '+this.status['status.'+status]);
    }
  }
}
